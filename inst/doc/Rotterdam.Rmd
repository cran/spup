...
In order to perform uncertainty propagation analysis using 'spup', the model through which uncertainty is propagated needs to be defined as an R function, for example:
...
Note that in a given the model has a requirement that the building function is stored in a SpatialPolygonDataFrame and in a column named 'Function'.
